...
- Normalización multilenguaje
- Clustering KMeans con detección de K (Silhouette)
- NUEVO: Embeddings SBERT (Sentence Transformers) para máxima precisión semántica.
- NUEVO: Prompt de Gemini optimizado para SEO, EEAT y Content Pillars.
"""
import os
import re
import unicodedata
...
# ------------------------
# Nombrado con Gemini (SEO Expert Mode)
# ------------------------
def name_clusters_seo_mode(df: pd.DataFrame, api_key: str, lang: str) -> pd.DataFrame:
    ...
    for i, (cid, group) in enumerate(groups):
        ...
        status.text(f"Analizando Cluster {cid} con enfoque SEO...")
...
st.markdown("""
Esta herramienta agrupa keywords basándose en **intención semántica profunda** y utiliza AI para definir pilares de contenido de alta autoridad.
""")
...
if use_gemini and gemini_key:
    df = name_clusters_seo_mode(df, gemini_key, "Español") # Default a Español para output SEO
else:
